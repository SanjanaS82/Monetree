...
import { LayoutDashboard, Building2, PiggyBank, Lock, ArrowRightLeft, List, User } from "lucide-react";
        ...
        <main className="max-w-5xl mx-auto px-4 py-6">
          <Tabs defaultValue="dashboard" className="space-y-6">
            <TabsList className="w-full flex flex-wrap h-auto gap-1 bg-muted p-1.5 rounded-lg">
              {tabs.map(tab => (
                <TabsTrigger key={tab.value} value={tab.value} className="flex items-center gap-1.5 text-xs sm:text-sm px-3 py-2">
